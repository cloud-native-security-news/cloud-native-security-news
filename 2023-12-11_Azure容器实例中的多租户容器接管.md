...
CaaS 产品的安全性探究一直颇具挑战。用户通常只能接触到自己的容器环境，并且通过防火墙限制了对本地网络的访问。为了更深入地理解 CaaS 平台是如何运行租户容器，Unit 42 使用 [WhoC](https://github.com/twistlock/whoc) ，一个特殊的容器映像，它能够读取执行它的容器运行环境。它基于是 Linux 容器中一个不太为人所知的设计缺陷，这个缺陷允许容器读取底层宿主机的容器运行时。这个想法类似于 CVE-2019-5736 漏洞，但区别在于我们的目的是读取宿主的运行时，而不是修改它。
...
当发现 ACI 中使用了这个旧版本的 runC 后，Unit 42 拿出了当时开发的 PoC 容器映像，进行了改进，部署到了 ACI 上并成功逃离了容器。在底层K8S Node节点宿主机获得了一个以 root 身份 Shell。
...
Unit 42 部署了一些逃逸容器，并发现它们落在了不同的 Kubernetes 集群上。我们发现，每个集群都有一个独特的集群 ID，这些 ID 在 1 到 125 之间变化，这说明每个地区（如西欧）可能托管了几十个不同的集群。
...
CVE-2018-1002102 揭示了 api-server 与 Kubelets 通信时存在运行重定向漏洞。通过重定向 api-server 的请求到另一个节点的 Kubelet，一个恶意的 Kubelet 可以在集群中利用此漏洞。图 8 显示了这个漏洞的基本流程：
...
接着创建了一个利用 CVE-2018-1002102 的自定义 Kubelet 镜像，将接收到的 exec 请求重定向到其他节点上的 pod。为了提高危害，Unit 42 将其配置为目标指向 api-server pod，然后运行 `az container exec my-ctr --exec-command /bin/bash`，期望在 api-server 容器上建立一个 shell，但这个命令失败了。

经过一些调试，Unit 42 发现重定向操作仅在目标容器和当前容器位于同一节点上时才有效。这实际上使得攻击无法影响其他节点。看了一下 [CVE-2018-1002102 的补丁](https://github.com/kubernetes/kubernetes/pull/66516)后，发现漏洞修复就是这样。但奇怪的是 api-server 是存在漏洞版本，不明白为啥已经修复了。

再次确定发送到节点的 exec 请求，我们原本预期这些请求会来自 api-server 的 IP，如图 8 所示。但这些请求实际上来自一个在默认命名空间运行的名为 k8s-agentpool1（下文用 'bridge' 代称） 的 pod。
...
Kubernetes 服务账户令牌是未加密的 JSON Web 令牌（JWTs），[因此它们可以被解码](https://jwt.io/)。从下面的内容可以看出，收到的令牌属于 'bridge' 服务账户。鉴于请求是从 bridge pod 发起的，这似乎合情合理。
...
所以在 k8s 中要注意服务账户令牌发送给了谁：任何接收到令牌的人都可以使用它，并冒充其拥有者。一般拿到 token 都会通过 `kubectl auth can-i` 命令 查看 token 的权限
